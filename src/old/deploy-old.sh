...
SAMPLE_HTML="<!DOCTYPE html><html><head><title>Success</title></head><body><h1>Hello, World!</h1><p>Your static site is ready and routing!</p></body></html>"
...
confirm() {
  ...
  if ! read -r -t 30 answer; then
    log "WARN" "Confirmation timed out, assuming default (Y)"
    return 0
  fi
...
create_acm_certificate() {
  ...
  if confirm "Create new ACM certificate for '$domain'?"; then
    ...
    # Wait until ACM has populated DomainValidationOptions
    log "INFO" "Waiting for ACM to generate the DNS validation CNAME..."
    for i in {1..12}; do     # up to ~120 s
      log "INFO" "Attempt $i: Waiting for ACM to generate the DNS validation CNAME..."
      cert_details=$(aws acm describe-certificate \
        --certificate-arn "$cert_arn" \
        --region us-east-1 \
        $AWS_PROFILE_ARG)
      record_name=$(echo "$cert_details" | jq -r '.Certificate.DomainValidationOptions[0].ResourceRecord.Name // empty')
      [ -n "$record_name" ] && break
      sleep 10
    done
    if [ -z "$record_name" ]; then
      log "ERROR" "ACM never returned a validation record. Try again later."
      return 1
    fi
...
create_origin_access_control() {
  ...
  log "INFO" "Hi! create_origin_access_control() is happening"
...
create_route53_record() {
  ...
  if confirm "Create Route53 record for '$domain' pointing to CloudFront?"; then
    ...
    log "INFO" "Domain: '$domain', Distribution: '$distribution_domain', Zone ID: '$zone_id'"
    
    # Create A record alias to CloudFront - ensure domain has trailing period
    local domain_with_period="${domain}."
    local change_batch="{
      \"Changes\": [
        {
          \"Action\": \"UPSERT\",
          \"ResourceRecordSet\": {
            \"Name\": \"$domain_with_period\",
            \"Type\": \"A\",
            \"AliasTarget\": {
    ...
    # Debug output
    log "INFO" "Change batch: $change_batch"
...
wait_for_distribution() {
  ...
  if confirm "Wait for CloudFront distribution to deploy? This may take 5-10 minutes."; then
    ...
    for attempt in {1..5}; do
      if aws cloudfront wait distribution-deployed --id "$distribution_id" "$AWS_PROFILE_ARG" "$AWS_REGION_ARG"; then
        log "SUCCESS" "Distribution deployed."
        break
      else
        log "WARN" "Attempt $attempt failed, retrying in 30s..."
        sleep 30
      fi
    done
...
main() {
  ...
  log "INFO" "Starting AWS Static Site Deployment Script -- confirming settings"
  ...
  log "INFO" "Parsed arguments: DOMAIN=$DOMAIN, AWS_PROFILE=$AWS_PROFILE, AWS_REGION=$AWS_REGION, STATUS_FILE=$STATUS_FILE"
